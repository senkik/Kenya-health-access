...
class USSDHandler:
    ...
    def process_input(self, text_array, last_input):
        ...
        elif menu_level == 'services_menu':
            return self.handle_services_menu(text_array, last_input)
...
Chagua nambari:"""
    ...
    def search_menu(self):
        """Search facility menu - expanded with new filter options"""
        return """CON Tafuta kwa:
1. Jina la Kaunti
2. Jina la Hospitali
3. Aina ya Huduma
4. Kata/Mtaa (Constituency)
5. Mji/Eneo (Town/Area)
6. Karibu nami (GPS)
7. Huduma maalum (Services)
8. 24/7 tu
...
Chagua nambari:"""
    ...
    def handle_search_flow(self, text_array, last_input):
        ...
        if search_step == 1:
            if last_input == "1":
                ...
                self.session['search_type'] = 'constituency'
                self.session['search_step'] = 2
                return "CON Andika jina la kata/mtaa (constituency):"
            elif last_input == "5":
                self.session['search_type'] = 'town'
                self.session['search_step'] = 2
                return "CON Andika jina la mji au eneo lako:"
            elif last_input == "6":
                # GPS-based search - trigger location request via Celery
                from .tasks import process_location_request
                phone_number = self.session.get('phone_number')
                process_location_request.delay(phone_number)
                return "END Tunaangalia eneo lako... tafadhali subiri. Utapokea SMS na hospitali karibu nawe."
            elif last_input == "7":
                # Services sub-menu
                self.session['menu_level'] = 'services_menu'
                self.session['search_step'] = 1
                return self.services_sub_menu()
            elif last_input == "8":
                # 24/7 only facilities
                self.session['search_type'] = '24hr'
                self.session['search_step'] = 3  # Skip text input, go straight to results
                facilities = self.search_facilities('24hr', '')
                self.session['search_results'] = self._build_results_list(facilities)
    ...
    def services_sub_menu(self):
        """Services sub-menu for filtering by specific service type"""
...
Chagua nambari (1-7):"""
    ...
    def handle_services_menu(self, text_array, last_input):
        """Handle services sub-menu selection"""
        service_map = {
            "1": "Emergency",
            "2": "Maternity",
            "3": "Pharmacy",
        ...
        if last_input in service_map:
            service_name = service_map[last_input]
            self.session['menu_level'] = 'search'
            self.session['search_type'] = 'specific_service'
            self.session['search_step'] = 3

            facilities = self.search_facilities('specific_service', service_name)
            self.session['search_results'] = self._build_results_list(facilities)
            return self.format_search_results()

        return "END Chaguo sio sahihi."

    def _build_results_list(self, facilities):
        """Build a serializable list of facility dicts from a queryset."""
        return [
            {
                'id': fac.id,
                'name': fac.name,
                'phone': fac.phone or 'N/A',
                'county': fac.county.name if hasattr(fac, 'county') and fac.county else getattr(fac, 'county_name', ''),
                'town': fac.town or '',
                'constituency': fac.constituency or '',
                'is_24_hours': fac.is_24_hours,
                'status': getattr(fac, 'availability_status', 'available'),
                'services': [s.name for s in fac.services.all()[:3]] if hasattr(fac, 'services') else []
            }
    ...
    def search_facilities(self, search_type, query):
        ...
        if search_type == 'county':
            ...
            facilities = facilities.filter(constituency__icontains=query)
        elif search_type == 'specific_service':
            facilities = facilities.filter(services__name__icontains=query).distinct()
        elif search_type == '24hr':
            facilities = facilities.filter(is_24_hours=True)
    ...
    def format_search_results(self):
        ...
        for i, fac in enumerate(results, 1):
            ...
            hr_tag = " [24/7]" if fac.get('is_24_hours') else ""
            response += f"{i}. {fac['name']} {icon}{hr_tag}\n"
    ...
    def send_facility_sms(self, facility):
        ...
        hr_str = "Ndiyo" if facility.get('is_24_hours') else "Hapana"
        constituency_str = facility.get('constituency', 'N/A') or 'N/A'
        
        message = (
            f"HudumaAfya: Maelezo ya {facility['name']}\n"
            f"Kaunti: {facility['county']}\n"
            f"Kata: {constituency_str}\n"
...
Kata: {facility.get('constituency', 'N/A')}
